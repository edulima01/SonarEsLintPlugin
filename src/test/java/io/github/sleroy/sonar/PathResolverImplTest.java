...
public class PathResolverImplTest {
	...
	public void returnsNull_ifRequestedPathDoesNotExist() {
		...
		final Optional<String> pathFromSetting = resolver.getPathFromSetting(sensorContext, "new path key",
				"notexisting.ts");
		assertFalse(pathFromSetting.isPresent());
	}
	...
	public void setUp() throws Exception {
		...
		final DefaultInputFile file = TestInputFileBuilder.create("bla", "existing.ts")
				.setLanguage(EsLintLanguage.LANGUAGE_KEY).build();
	 ...
	 * converted in File objects to avoid case-sensitive issues on some filesystems.
	 *
