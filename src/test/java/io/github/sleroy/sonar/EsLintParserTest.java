...
public class EsLintParserTest {
	...
	public void eslint_parsingFailure() throws IOException {
		final String parseRow1 = FileUtils.readFileToString(new File("src/test/resources/results/parsingFailure.json"),
		...
		assertEquals("Expected one error, parsing error", 1,
				issues.get("c:/workspace/SonarTsPlugin/src/test/resources/angular.html").size());
	}

	/**
	 * Tests the output of EsLint when no results are returned and the analysis is a
	 * success.
	 */
	@Test
	...
	public void eslint_successWithResults() throws IOException {
		final String parseRow1 = FileUtils.readFileToString(new File("src/test/resources/results/ok.json"),
				Charset.defaultCharset());
		final List<String> toParse = new ArrayList<>();
		toParse.add(parseRow1);
		...
		assertEquals("Expected fifty-eight violations", 58,
				issues.get("c:/workspace/SonarTsPlugin/src/test/resources/dashboard.js").size());
	}
